...
import {startEmotionTab, stopEmotionTab} from './emotion.js';
...
function switchTab(tab){currentTab=tab;document.body.dataset.tab=tab;$$('.tab-btn').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));if(tab==='setup'&&playing){playing=false;$('#play-btn').textContent='▶ Run live';$('#stage-status').textContent='PAUSED'}if(tab==='setup'){tool='select';$$('.tools button').forEach(b=>b.classList.toggle('active',b.dataset.tool==='select'));$('#stage-status').textContent='EDIT MODE';selected=null;renderInspector()}if(tab==='simulate'){selected=null;renderInspector();$('#stage-status').textContent=simulation&&!dirty?(playing?'RUNNING LIVE':'READY TO RUN'):'READY TO RUN'}if(tab==='dashboard'){loadDashboard()}if(tab==='emotion'){startEmotionTab()}else{stopEmotionTab()}requestAnimationFrame(()=>{resizeCanvas();draw()})}
