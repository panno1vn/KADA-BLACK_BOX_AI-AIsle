...
function createSimulation(seed=createRunSeed(lastRunSeed)){lastRunSeed=seed;const pop=population(seed);simulation=new LiveSimulation({layout,catalog,population:pop,parameters,seed,durationMinutes:Number($('#duration').value)});npcRenderer.reset(seed,performance.now());simResult=null;dirty=false;selected=null;lastPurchaseCount=0;lastFinishedCount=0;updateMetrics();renderInspector();renderEvents();draw();return simulation}
